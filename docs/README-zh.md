...
打开 [http://127.0.0.1:4000](http://127.0.0.1:4000)，保持终端运行。修改并保存页面或样式后，Jekyll 会自动编译并刷新浏览器；按 `Ctrl+C` 停止服务。

- 简历正文和论文列表：`_pages/about.md`
- 姓名、邮箱、头像等个人信息：`_config.yml`（修改后需停止并重新运行服务）
- 顶部导航：`_data/navigation.yml`
- 页面样式：`assets/css/main.scss` 和 `_sass/`
- 图片：`images/`

本地依赖放在 `vendor/`，构建结果放在 `_site/`，这些目录已加入 `.gitignore`，上传到 GitHub 时无需包含。如果使用 GitHub 网页手动上传文件，也请跳过这些目录及 `.bundle/`、`.sass-cache/` 等缓存目录。
